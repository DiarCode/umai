...
const addToCart = () => {
  ...
  console.log(`Добавлено блюдо ${dish.value.name} в корзину`)
}
...
<template>
  <div class=" p-4">
    <div class="relative h-60 bg-gray-200 rounded-xl overflow-hidden flex items-center justify-center mb-4">
      <img 
        v-if="dish?.image" 
        :src="dish.image" 
        :alt="dish.name" 
    ...
    <div class="flex gap-4 mb-4">
      <button class="flex-1 bg-blue-500 text-white rounded-4xl px-4 py-2 flex items-center justify-center">
        <img src="../../../assets/AI_icon.svg" alt="AI" class="w-6 h-6" />
        <span class="ml-2">AI помощник</span>
      </button>

      <button class="flex-1 bg-black text-white rounded-4xl px-4 py-2 flex items-center justify-center">
    ...
    <button 
      class="fixed bottom-4 left-4 right-4 bg-black text-white py-4 rounded-2xl font-semibold95"
      @click="addToCart"
    >
      {{ dish?.price }} ₸
    </button>
